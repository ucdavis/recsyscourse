...
\begin{lstlisting}
> library(recosystem)
# recosystem uses 'R6' R objects; all action will take place within r;
...
> ml.trn <- data_memory(ml[,1],ml[,2],ml[,3],index1=TRUE)
\end{lstlisting}

\begin{lstlisting}
# do the factorization, with rank 20; do use NMF
> r$train(ml.trn,opts=list(dim=20,nmf=TRUE))
iter      tr_rmse          obj
   0       2.0381   5.0056e+05
   1       1.0296   1.7402e+05
   2       0.9529   1.6028e+05
...
> preds <- r$predict(ml.trn,out_memory())
> head(preds)
