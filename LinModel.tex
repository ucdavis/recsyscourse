...
real line, $A = (c,c+\epsilon)$.  Let $q = P(Y = 1)$, the
\textit{unconditional probability of class 1}.  Let
$w_i(t)$ denote the conditional density of $X$, given $Y = i$ (which by
assumption is normal).  Then by Bayes Rule,
...
As noted, we can also predict general categorical $Y$, .e.g.

\begin{lstlisting}
> qeo <- qeLogit(mlb,'Position')
holdout set has  101 rows
> predict(qeo,data.frame(Height=73,Weight=225,Age=25))
$predClasses
[1] "Relief_Pitcher"
...
[1,] 0.1475545    0.09828611  0.2300844       0.245291     0.02082625
      Shortstop Starting_Pitcher Third_Baseman
[1,] 0.01016817        0.1768094     0.0709802
\end{lstlisting}

The most likely position played by this person is Relief Pitcher, though
others have probabilities almost as high.  In fact, let's ask how well
...
30\% of the time, not that good but then we are predicting among eight
